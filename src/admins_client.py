...
def create_admins_client(app):
    ...
    def add_admin(course):
        ...
        with connect_db() as db:
            check = db("SELECT * FROM course_admins WHERE email=(%s) AND course=(%s)", [get_email(app.remote), course]).fetchone()
        if not check:
            db("INSERT INTO course_admins VALUES (%s, %s, %s, %s)", [get_email(app.remote), get_name(app.remote), course, get_name(app.remote)])
